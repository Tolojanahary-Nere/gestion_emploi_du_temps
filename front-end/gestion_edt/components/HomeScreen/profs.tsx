...
const Profs: React.FC = () => {
  ...
  const ajouterProfesseur = async () => {
    setLoading(true);
    try {
      await axios.post('http://10.0.2.2:8000/api/Gestedt/ProfesseurList/', { nom, prenom, contact });
      fetchProfesseurs();
  ...
  const mettreAJourProfesseur = async () => {
    ...
    try {
      if (currentProfId !== null) {
        await axios.put(`http://10.0.2.2:8000/api/Gestedt/professeurs/${currentProfId}/`, { nom, prenom, contact });
        fetchProfesseurs();
        setModalVisible(false);
        setNom('');
        setPrenom('');
  ...
  const supprimerProfesseur = async (id: number) => {
    setLoading(true);
    try {
      await axios.delete(`http://10.0.2.2:8000/api/Gestedt/professeurs/${id}/`);
      fetchProfesseurs();
      setError(null);
      setShowAlert(true);
    } catch (error) {
