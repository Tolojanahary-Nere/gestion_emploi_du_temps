...
const Salle: React.FC = () => {
  ...
  const ajouterSalle = async () => {
    setLoading(true);
    try {
      await axios.post('http://10.0.2.2:8000/api/Gestedt/salles/', { nomSalle, batiment, lieu, est_libre: estLibre });
      fetchSalles();
  ...
  const mettreAJourSalle = async () => {
    ...
    try {
      if (currentSalleId !== null) {
        await axios.put(`http://10.0.2.2:8000/api/Gestedt/salles/${currentSalleId}/`, { nomSalle, batiment, lieu, est_libre: estLibre });
        fetchSalles();
        setModalVisible(false);
        setNomSalle('');
        setBatiment('');
  ...
  const supprimerSalle = async (id: number) => {
    setLoading(true);
    try {
      await axios.delete(`http://10.0.2.2:8000/api/Gestedt/salles/${id}/`);
      fetchSalles();
      setError(null);
      setShowAlert(true);
    } catch (error) {
