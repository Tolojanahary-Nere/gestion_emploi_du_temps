...
const Niveau: React.FC = () => {
  ...
  const ajouterNiveau = async () => {
    setLoading(true);
    try {
      await axios.post('http://10.0.2.2:8000/api/Gestedt/niveaux/', { nomNiveau, departement: departementId });
      fetchNiveaux();
  ...
  const mettreAJourNiveau = async () => {
    ...
    try {
      if (currentNiveauId !== null) {
        await axios.put(`http://10.0.2.2:8000/api/Gestedt/niveaux/${currentNiveauId}/`, { nomNiveau, departement: departementId });
        fetchNiveaux();
        setModalVisible(false);
        setNomNiveau('');
        setDepartementId('');
  ...
  const supprimerNiveau = async (id: number) => {
    setLoading(true);
    try {
      await axios.delete(`http://10.0.2.2:8000/api/Gestedt/niveaux/${id}/`);
      fetchNiveaux();
      setError(null);
      setShowAlert(true);
    } catch (error) {
