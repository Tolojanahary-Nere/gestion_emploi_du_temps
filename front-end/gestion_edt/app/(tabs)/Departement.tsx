...
const Departement: React.FC = () => {
  ...
  const ajouterDepartement = async () => {
    setLoading(true);
    try {
      await axios.post('http://192.168.43.238:8000/api/Gestedt/departements/', { nomDepartement });
      fetchDepartements();
  ...
  const mettreAJourDepartement = async () => {
    ...
    try {
      if (currentDeptId !== null) {
        await axios.put(`http://192.168.43.238:8000/api/Gestedt/departements/${currentDeptId}/`, { nomDepartement });
        fetchDepartements();
        setModalVisible(false);
        setNomDepartement('');
        setError(null);
  ...
  const supprimerDepartement = async (id: number) => {
    setLoading(true);
    try {
      await axios.delete(`http://192.168.43.238:8000/api/Gestedt/departements/${id}/`);
      fetchDepartements();
      setError(null);
      setShowAlert(true);
    } catch (error) {
